...
| `status_entry` | error | A record entered the graph at anything but its flow's entry status (registered only with a flow; diff-aware) |
...
`status_entry` measures that a record enters the graph, not that a person authored it. A node is its id, so a document moved under a path-derived id arrives as a new record and is reported as one — `nodex rename` anchors the id first and produces no arrival. **One flow per project**: a second lifecycle is a config-key change, deferred deliberately because `kinds` already expresses the requirement that arrived (kinds with *no* lifecycle) and the guards are already scoped per kind.
...
```bash
nodex --check-version ">=0.41, <0.42" build
```
