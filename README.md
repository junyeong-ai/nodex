...
| `status_transition` | error | A status moved somewhere `[statuses.flow]` does not declare, over the kinds that flow governs — a move out of a terminal status included (registered only with a flow; diff-aware) |
| `status_entry` | error | A record entered the flow at anything but its entry status (registered only with a flow; diff-aware) |
...
Declared, it registers `status_transition` and `status_entry`, and `lifecycle` / `scaffold --force` refuse a move it does not name at the write seam, without needing a baseline. Omitted, nothing is judged and `[statuses].terminal` stays the only statement nodex has about how a lifecycle ends.
...
Both rules judge history a step at a time: each commit `<ref>..HEAD` adds against its parents, and the uncommitted change against `HEAD` (and every `MERGE_HEAD` while a merge is under way). A range therefore reports exactly what a gate on each of its commits would — a record authored at `proposed` and accepted a commit later passes, and a detour through an undeclared status is reported at the commit that took it even when the range ends where a declared move would. A merge is judged only where it differs from every parent, so a branch brought in carries its own steps. `details.commit` names the step (absent for the uncommitted change). A write seam judges the step its write would commit, from `HEAD` rather than the baseline; `check --content` judges a proposal against the working tree, which is no commit, and reports both rules skipped.

What the rules measure is that a record enters the flow, not that a person authored it. A node is its id, so a record with no governed prior status on any parent is an entry however it got there: authored, moved under a path-derived id, re-keyed, given a governed kind, or back after a commit that deleted it or could not parse it — `nodex rename` anchors the id first and produces no arrival. Judging the history the repository keeps has two consequences: a squash merge folds a branch into one commit, so a record authored and accepted on it enters accepted; and a range a shallow clone does not hold is refused (`GIT_ERROR`) rather than read as every record arriving — fetch it (`fetch-depth: 0`). Each commit in the range costs a build, paid only where a flow is declared. **One flow per project**: a second lifecycle is a config-key change, deferred deliberately because `kinds` already expresses the requirement that arrived (kinds with *no* lifecycle) and the guards are already scoped per kind.
