...
Both rules judge history a step at a time, against git rather than `rules.immutable_baseline`: every `check`, `query issues` and write seam judges the uncommitted change against `HEAD` (and every `MERGE_HEAD` while a merge is under way), and `check --since <ref>` also judges each commit the heads reach and `<ref>` does not, against its parents. A range therefore reports what a gate on each of its commits would under today's config — a record authored at `proposed` and accepted a commit later passes, and a detour through an undeclared status is reported at the commit that took it even when the range ends where a declared move would. A merge is a step like any other — resolving one in favour of a line leaves a tree a parent already had, and that is still the step where the other line's record moved. What it was made on is what each line *moved* since the parents last agreed: a line that did not touch the record makes no claim about it, exactly as it makes no claim about a file it did not edit — so a branch forked before a record was superseded cannot carry the old status back through the merge. Reading where the lines agreed costs one more snapshot, and only where they disagree about some record; lines that have merged each other before agreed in more than one place and every one of them is read, since taking one would decide by whichever git listed first what the others say. Lines that share no commit — an `--allow-unrelated-histories` merge — leave a record they disagree about counted rather than judged, and so do places the lines agreed that disagree about it, a record a step holds two readings of, a commit whose tree the build refuses, and what a shallow clone's cut puts out of reach — a document's earlier state or a place the lines agreed — each named by a `history_unread` warning. `details.commit` names the step (absent for the uncommitted change). A step finding is about a commit, so a plain `check` never re-judges history: a past finding is reported by the `--since` range that holds it and by no later run. `check --content` judges a proposal against the working tree, which is no commit, and reports both rules skipped; outside a git work tree both skip.

What the rules measure is that a record enters the flow, not that a person authored it. A node is its id, so a record with no governed prior status on any parent is an entry however it got there: authored, moved under a path-derived id, re-keyed, given a governed kind, or restored after a commit that deleted it — `nodex rename` anchors the id first and produces no arrival. A document a commit could not parse holds the record it held before the change that broke it, read at the path it stands on: broken and repaired is the record it was, while one never readable before its repair enters there. A commit that both moves a document and leaves it unparseable takes the path's history with it, so the repair reads as an arrival — move it in a commit of its own. Where the clone is shallow and what the path held lies beyond the cut, neither rule judges the records that may have stood there; they are counted `unjudged` instead, and `fetch-depth: 0` judges them. A commit whose tree the build refuses under today's config — two documents whose ids resolve the same way while a merge holds both — is one the walk cannot read: it says which commit (`history_unread`) and counts the records that step carried rather than judging them, because nothing short of rewriting that commit would make it readable. A document git ignores takes no step, because no commit can hold it, and both rules count it `unjudged`; its writes are judged from the status it carries, as outside a git work tree. On a branch with no commit yet, the next commit records no parents, so every record it carries enters there. Judging the history the repository keeps has consequences: a squash merge folds a branch into one commit, so a record authored and accepted on it enters accepted — squash-merging repositories accept in a PR of its own; and a range a shallow clone does not hold is refused (`GIT_ERROR`) rather than read as every record arriving — fetch it (`fetch-depth: 0`). An amended commit records its step from `HEAD^`, while a `check` run before `git commit --amend` judged it from `HEAD`. Each run builds `HEAD`'s snapshot and, while a merge is under way, every `MERGE_HEAD` and every place the lines agreed beside it; each commit in a `--since` range costs one more, and a document a commit could not parse costs one per commit the read-back crosses, once each however many commits stand on it. Paid only where a flow is declared. **One flow per project**: a second lifecycle is a config-key change, deferred deliberately because `kinds` already expresses the requirement that arrived (kinds with *no* lifecycle) and the guards are already scoped per kind.
...
Every git-backed feature — the immutability baseline, `git_drift`, `diff`, `impact` — measures **the project**, at its own location inside the repository that tracks it. `git_drift` reads the history in one `git log` walk and so needs git 2.31 or newer; on an older git the walk is refused and every drift target reports unmeasurable rather than being mismeasured. A `nodex.toml` in a subdirectory of a larger repository is as ordinary as one at the repository root: paths are read against the project's own prefix, and a materialised ref is graphed from the project's directory inside the checkout, not from the repository around it. The binding is resolved once per command and named outright, so nothing in the ambient environment can move the target — an inherited `GIT_DIR`, the quarantine object directory a server-side hook exports, or a pathspec-magic variable changes nothing about what nodex measures.
...
```bash
nodex --check-version ">=0.43, <0.44" build
```
