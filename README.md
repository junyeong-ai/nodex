...
| `nodex impact <ref-a> <ref-b> [--depth N --relations a,b]` | "What breaks if I merge this?" — the diff plus each modified (edited or moved) node's transitive dependents and each removed node's direct referrers that still point at it (now dangling), with a `likely_breaking` list of removed nodes the *after* graph still references |
...
| `nodex query orphans [--limit N]` | Live nodes with zero external incoming edges, outside `orphan_ok_kinds`, per-node `orphan_ok` and `orphan_grace_days` (self-links don't count) — the same population the `orphan` rule guards |
