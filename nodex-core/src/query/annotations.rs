...
/// captured key. Patterns whose extraction yielded no markers are
...
/// within a group, markers are sorted by `key`; within a marker,
...
mod tests {
    ...
    fn groups_by_key_and_counts() {
        ...
        assert_eq!(groups[0].markers.len(), 2);
        let spec_x = groups[0]
            .markers
            .iter()
            .find(|m| m.key == "spec-x")
    ...
    fn sources_are_sorted_by_source_id_then_line() {
        ...
        let groups = find_annotations(&g, None);
        let sources = &groups[0].markers[0].sources;
    ...
    fn source_path_resolved_from_node() {
        ...
        let groups = find_annotations(&g, None);
        assert_eq!(groups[0].markers[0].sources[0].path, "docs/doc-1.md");
    }
}
