...
pub enum RuleSource {
    /// Rule code is part of nodex (e.g. `schema/required`,
...
pub fn export_rules(config: &Config) -> RulesManifest {
    ...
    // unknown_field is only meaningful in strict mode; in lenient
...
mod tests {
    ...
    fn rules_manifest_omits_strict_only_rule_in_lenient_mode() {
        let m = export_rules(&Config::default());
        assert!(
            !rule_ids(&m).contains(&"schema/unknown_field"),
    ...
    fn rules_manifest_includes_strict_only_rule_in_strict_mode() {
        ...
        assert!(rule_ids(&export_rules(&c)).contains(&"schema/unknown_field"));
    }
