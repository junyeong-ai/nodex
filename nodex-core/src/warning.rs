...
pub enum WarningCode {
    ...
    /// rank by: a target carrying no positively-weighted signal, which ranks
    /// nothing (`query similar`), or no composite over a complete basis — a
    /// positively-weighted component the run can measure and the document
    /// declares no input for, or none of them present at all (`query trust
    ...
    ///
    /// Either something stood between the command and the edit — a symlink,
    /// an immutability lock, an unclosed fence, an unreadable path, a
    /// mid-flight change, no acceptable rewrite — and what it would have
    /// changed stands as it was. A `retarget` says which reference kept its
    /// target; a `migrate` says which file went unwritten. Or the write landed
    ...
    /// on the next build as an unresolved edge that `query issues` counts —
    /// never as a `check` error, because a project whose
    /// `[[detection.unresolved_policy]]` calls that cause an error is one
    /// whose write gate refuses the move outright, and then there is no
    /// warning to read. So the warning says the command gave up on a
    /// reference the project tolerates losing, a build before anything else
    /// mentions it.
    FileSkipped,
