...
pub fn scaffold(
    ...
    if !scan.paths.contains(&rel_path) {
        ...
        let cause = if let Some(cause) = undescended.as_deref() {
            ...
            "a [[scope.conditional_exclude]] rule drops the child_glob matches in a terminal \
    ...
    // corpus is not evidence that this is a new project — it is equally a
    // corpus behind a boundary the walk did not cross, or one whose every
...
pub fn render_default_frontmatter(
    ...
    lines.push(format!(
        "status: {}",
        crate::yaml_text::quote(config.initial_status_for(kind))
    ));
