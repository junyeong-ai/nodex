...
use crate::model::{Annotation, Graph, Node, RawAnnotation, RawEdge};
...
pub fn build(root: &Path, config: &Config, full_rebuild: bool) -> Result<BuildResult> {
    ...
    let mut cached_results: Vec<(Node, Vec<RawEdge>, Vec<RawAnnotation>)> = Vec::new();
    ...
    for (node, raw_edges, raw_annotations) in cached_results {
    ...
    Ok(BuildResult {
        graph: Graph::new(node_map, edges, annotations),
        stats,
        warnings,
    })
}
...
mod tests {
    use super::*;
    use crate::config::{AnnotationConfig, KindsConfig};
