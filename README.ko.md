...
두 rule 은 `rules.immutable_baseline` 이 아니라 git 이력을 한 걸음씩 판정한다: 모든 `check`·`query issues`·write seam 은 커밋되지 않은 변경을 `HEAD` 에 (merge 진행 중이면 모든 `MERGE_HEAD` 에도) 대고 판정하고, `check --since <ref>` 는 여기에 더해 head 들이 도달하고 `<ref>` 는 도달하지 않는 커밋 각각을 그 부모들에 대고 판정한다. 그래서 구간의 판정은 현재 config 기준으로 각 커밋에 게이트를 걸었을 때의 판정과 같다 — `proposed` 로 작성하고 다음 커밋에서 승인한 레코드는 통과하고, 선언되지 않은 status 를 거친 우회는 구간이 선언된 이동과 같은 곳에서 끝나더라도 그 걸음을 만든 커밋에서 보고된다. merge 는 모든 부모와 다른 곳만 판정하며, 그 걸음이 딛고 선 것은 각 갈래가 부모들이 마지막으로 일치한 지점 이후에 *움직인* 것이다. 레코드를 건드리지 않은 갈래는 자신이 편집하지 않은 파일에 대해 그렇듯 그 레코드에 대해 아무 주장도 하지 않으므로, 레코드가 superseded 되기 전에 갈라진 브랜치가 merge 를 통해 옛 status 를 되가져올 수 없다. 갈래들이 일치한 지점을 읽는 데는 스냅샷이 하나 더 들지만, 어떤 레코드에 대해 갈래들이 서로 다를 때만 읽는다. `details.commit` 이 걸음을 가리킨다(커밋되지 않은 변경이면 없음). 걸음의 발견은 커밋에 대한 것이므로 일반 `check` 는 이력을 다시 판정하지 않는다: 지난 발견은 그것을 담은 `--since` 구간이 보고하고, 이후 실행은 보고하지 않는다. `check --content` 는 커밋이 아닌 작업 트리에 대고 제안을 판정하므로 두 rule 을 skipped 로 보고하고, git 작업 트리 밖에서도 두 rule 은 skip 한다.
