...
| `status_entry` | error | 레코드가 진입 status 가 아닌 곳으로 flow 에 진입 (flow 가 있을 때만 등록, diff-aware) |
...
두 rule 은 이력을 한 걸음씩 판정한다: `<ref>..HEAD` 가 더한 커밋 각각을 그 부모들에 대고, 커밋되지 않은 변경은 `HEAD` 에 (merge 진행 중이면 모든 `MERGE_HEAD` 에도) 대고 판정한다. 그래서 구간의 판정은 각 커밋에 게이트를 걸었을 때의 판정과 정확히 같다 — `proposed` 로 작성하고 다음 커밋에서 승인한 레코드는 통과하고, 선언되지 않은 status 를 거친 우회는 구간이 선언된 이동과 같은 곳에서 끝나더라도 그 걸음을 만든 커밋에서 보고된다. merge 는 모든 부모와 다른 곳만 판정하므로, 합쳐 들어온 브랜치의 걸음은 그 브랜치의 커밋이 답한다. `details.commit` 이 걸음을 가리킨다(커밋되지 않은 변경이면 없음). write seam 은 자기 쓰기가 커밋할 걸음을 baseline 이 아니라 `HEAD` 에서 판정하고, `check --content` 는 커밋이 아닌 작업 트리에 대고 제안을 판정하므로 두 rule 을 skipped 로 보고한다.

두 rule 이 측정하는 것은 레코드가 flow 에 진입했다는 사실이지 사람이 거기서 작성했다는 것이 아니다. node 는 곧 id 이므로, 어느 부모에서도 flow 가 지배하는 이전 status 가 없는 레코드는 경위와 무관하게 진입이다: 작성, 경로에서 유도된 id 로의 이동, id 변경, 지배받는 kind 로의 변경, 삭제했거나 parse 하지 못한 커밋 뒤의 복귀 — `nodex rename` 은 먼저 id 를 frontmatter 에 고정하므로 진입이 발생하지 않는다. 저장소가 보존한 이력을 판정하므로 두 가지가 따른다: squash merge 는 브랜치를 커밋 하나로 접으므로 그 안에서 작성하고 승인한 레코드는 승인된 채로 진입하고, shallow clone 이 담지 못한 구간은 모든 레코드가 새로 들어온 것으로 읽는 대신 거부된다(`GIT_ERROR`) — 이력을 가져오면 된다(`fetch-depth: 0`). 구간의 커밋마다 빌드가 한 번씩 들며, flow 를 선언한 프로젝트만 그 비용을 낸다. **프로젝트당 flow 는 하나**: 두 번째 생명주기는 config 키 변경이며, `kinds` 가 이미 도착한 요구(생명주기가 *없는* kind)를 표현하고 guard 가 이미 kind 별로 좁혀져 있으므로 의도적으로 미뤘다.
