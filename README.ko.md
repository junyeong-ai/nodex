...
두 rule 은 `rules.immutable_baseline` 이 아니라 git 이력을 한 걸음씩 판정한다: 모든 `check`·`query issues`·write seam 은 커밋되지 않은 변경을 `HEAD` 에 (merge 진행 중이면 모든 `MERGE_HEAD` 에도) 대고 판정하고, `check --since <ref>` 는 여기에 더해 head 들이 도달하고 `<ref>` 는 도달하지 않는 커밋 각각을 그 부모들에 대고 판정한다. 그래서 구간의 판정은 현재 config 기준으로 각 커밋에 게이트를 걸었을 때의 판정과 같다 — `proposed` 로 작성하고 다음 커밋에서 승인한 레코드는 통과하고, 선언되지 않은 status 를 거친 우회는 구간이 선언된 이동과 같은 곳에서 끝나더라도 그 걸음을 만든 커밋에서 보고된다. merge 도 다른 걸음과 같이 판정한다. 한쪽 갈래를 택해 해소하면 트리는 그 부모의 것과 같아지지만, 다른 갈래의 레코드가 움직인 걸음은 여전히 그 merge 다. 그 걸음이 딛고 선 것은 각 갈래가 부모들이 마지막으로 일치한 지점 이후에 *움직인* 것이다. 레코드를 건드리지 않은 갈래는 자신이 편집하지 않은 파일에 대해 그렇듯 그 레코드에 대해 아무 주장도 하지 않으므로, 레코드가 superseded 되기 전에 갈라진 브랜치가 merge 를 통해 옛 status 를 되가져올 수 없다. 갈래들이 일치한 지점을 읽는 데는 스냅샷이 하나 더 들지만, 어떤 레코드에 대해 갈래들이 서로 다를 때만 읽는다. 서로를 merge 한 적이 있는 갈래들은 일치한 지점이 둘 이상이며 그 전부를 읽는데, 하나만 취하면 나머지가 그 레코드에 대해 말하는 바를 git 이 먼저 나열한 순서가 결정하기 때문이다. 공통 커밋이 없는 갈래들(`--allow-unrelated-histories` merge)이 서로 다르게 말하는 레코드는 판정하지 않고 센다. 갈래들이 일치한 지점이 둘 이상이고 그 지점들이 레코드에 대해 서로 다르게 말하는 경우도 그렇다. 한 걸음이 두 가지로 읽는 레코드, 빌드가 트리를 거부하는 커밋이 담은 레코드, 얕은 클론의 cut이 닿지 못하게 하는 것 — 문서의 이전 상태든 갈래들이 일치한 지점이든 — 이 걸린 레코드도 그렇다. 각각은 무엇을 읽지 못했고 무엇을 하면 되는지 말하는 `history_unread` 경고를 함께 낸다. `details.commit` 이 걸음을 가리킨다(커밋되지 않은 변경이면 없음). 걸음의 발견은 커밋에 대한 것이므로 일반 `check` 는 이력을 다시 판정하지 않는다: 지난 발견은 그것을 담은 `--since` 구간이 보고하고, 이후 실행은 보고하지 않는다. `check --content` 는 커밋이 아닌 작업 트리에 대고 제안을 판정하므로 두 rule 을 skipped 로 보고하고, git 작업 트리 밖에서도 두 rule 은 skip 한다.

두 rule 이 측정하는 것은 레코드가 flow 에 진입했다는 사실이지 사람이 거기서 작성했다는 것이 아니다. node 는 곧 id 이므로, 어느 부모에서도 flow 가 지배하는 이전 status 가 없는 레코드는 경위와 무관하게 진입이다: 작성, 경로에서 유도된 id 로의 이동, id 변경, 지배받는 kind 로의 변경, 삭제한 커밋 뒤의 복원 — `nodex rename` 은 먼저 id 를 frontmatter 에 고정하므로 진입이 발생하지 않는다. 커밋이 parse 하지 못한 문서는 그 문서를 깨뜨린 변경 직전에 담고 있던 레코드로 판정하며, 문서가 서 있는 경로에서 읽는다. 깨졌다가 복구된 레코드는 원래의 레코드로 판정되고, 복구 전에 한 번도 읽힌 적이 없는 레코드는 복구한 커밋에서 진입한다. 한 커밋에서 문서를 옮기면서 parse 되지 않는 상태로 남기면 그 경로의 이력이 함께 옮겨지지 않으므로 복구가 진입으로 읽힌다 — 이동은 별도 커밋으로 하면 된다. clone 이 shallow 여서 그 경로가 담고 있던 것이 절단면 너머에 있으면 두 rule 모두 거기 서 있었을 수 있는 레코드를 판정하지 않고 `unjudged` 로 세며, `fetch-depth: 0` 이면 판정한다. 오늘의 config 로 빌드가 거부하는 트리를 담은 커밋 — merge 가 양쪽을 함께 담아 id 가 같게 해석되는 문서 둘이 생긴 경우 — 은 walk 가 읽지 못하는 커밋이다. 어느 커밋인지 알리고(`history_unread`) 그 걸음이 담은 레코드는 판정하지 않고 세는데, 그 커밋을 다시 쓰지 않는 한 읽을 수 있게 만들 방법이 없기 때문이다. git 이 무시하는 문서는 어떤 커밋에도 담기지 않으므로 걸음을 만들지 않고 두 rule 모두 `unjudged` 로 세며, 그 문서에 대한 쓰기는 git 작업 트리 밖에서처럼 문서가 가진 status 에서 판정한다. 아직 커밋이 없는 브랜치에서는 다음 커밋이 부모를 기록하지 않으므로, 그 커밋이 담는 레코드는 모두 거기서 진입한다. 저장소가 보존한 이력을 판정하므로 따르는 것들이 있다: squash merge 는 브랜치를 커밋 하나로 접으므로 그 안에서 작성하고 승인한 레코드는 승인된 채로 진입한다 — squash merge 저장소는 승인을 별도 PR 로 올린다. shallow clone 이 담지 못한 구간은 모든 레코드가 새로 들어온 것으로 읽는 대신 거부된다(`GIT_ERROR`) — 이력을 가져오면 된다(`fetch-depth: 0`). `git commit --amend` 로 고친 커밋은 `HEAD^` 에서 걸음을 기록하지만, amend 전에 돌린 `check` 는 `HEAD` 에서 판정했다. 실행마다 `HEAD` 의 스냅샷을 빌드하고, merge 가 진행 중이면 모든 `MERGE_HEAD` 와 갈래들이 일치한 모든 지점을 함께 빌드한다. `--since` 구간이면 커밋마다 하나씩 더 들고, 커밋이 파싱하지 못한 문서가 있으면 되읽기가 거쳐 가는 커밋마다 하나씩 더 드는데 그 답은 커밋당 한 번만 구한다. flow 를 선언한 프로젝트만 그 비용을 낸다. **프로젝트당 flow 는 하나**: 두 번째 생명주기는 config 키 변경이며, `kinds` 가 이미 도착한 요구(생명주기가 *없는* kind)를 표현하고 guard 가 이미 kind 별로 좁혀져 있으므로 의도적으로 미뤘다.
...
`nodex.toml` 의 `[meta] nodex_version = ">=0.42, <0.43"` 은 프로젝트 문서를 **쓸** 수 있는 바이너리를 핀. 요구를 벗어난 바이너리에서도 읽기 명령은 실행되며 envelope `warnings` 에 비치명적 경고를 첨부하고, 문서를 쓰는 명령(`scaffold`, `migrate --apply`, `rename`, `retarget`, `lifecycle`)만 `VERSION_MISMATCH` 로 거부 — 그래프 읽기는 손상시킬 수 없으므로 변형만 게이트. 모든 CI / 컨트리뷰터가 자체 검사를 다시 짤 필요 없이 도구 버전을 핀. 글로벌 `--check-version` CLI 플래그는 불일치 시 *모든* 명령을 거부하는 별도 하드 게이트.
