...
- `statuses.flow` — one declared lifecycle: `transitions` (the targets each status may move to) over `kinds` (which kinds have that lifecycle at all; empty = every kind). Optional; omitted, nothing is judged. Declared, it is the one place the flow is written and `Config::validate` holds the three declarations to each other, scoped to what a governed kind can hold: a terminal status names no transition, a non-terminal one names at least one, and each is reachable from `initial` — otherwise it is vocabulary the `status` enum accepts and no document could legally hold. It registers `status_transition` (a move the flow does not name, a move out of terminal included) and `status_entry` (a document authored into anything but `initial`), which split the governed corpus: the first guards the records the baseline holds, the second the ones it does not, and neither judges a re-keyed record, whose prior record is gone under another id. `kinds` is load-bearing rather than convenience — a kind written live has no acceptance event, and a flow without the filter would demand a promotion step it never has
