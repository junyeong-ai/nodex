...
- `scope.conditional_exclude[]` — drop the sub-artifacts governed by a terminal parent (`parent_glob` selects the parent, `child_glob` selects which paths are derivative; only `child_glob` matches are excluded; the dropped paths are reported on the build result, and the write that drops them names them as `document_evicted`). The **directory subtree** is the unit: one terminal parent governs every `child_glob` match beside it, including the sub-artifacts of parents that are still live, because nothing pairs a child with one parent by name. A project needing per-record eviction gives each record its own directory
