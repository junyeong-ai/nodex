...
- Exit code 0 = success, 1 = `check` found Error-severity violations, 2 = every error envelope (config, parse, IO, version, CLI-arg, runtime) and every envelope stdout would not take (a pipe closed early, a full disk) — a verdict that was not delivered never exits as one that was. The encoder (`envelope::print_json`) owns that exit, so no command can outlive a failed write
