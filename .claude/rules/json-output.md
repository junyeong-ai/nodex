...
- A verdict answers for what was judged, never for what is displayed. `check --severity` narrows the listed violations; `has_errors` and the exit code stay drawn from the whole judged set, and what the filter hid is disclosed as `gate_suppression`. A presentation knob that moved the verdict would be a gate certifying something other than what it checked — the same defect as a rule reporting green over a population it never ran on
