...
A `file_skipped` warning from `rename` splits by what the reference names afterwards. A **different valid document** is reported nowhere else: the write succeeded and the graph is valid, so the warning is the only record. **Nothing** is reported again on the next build as an unresolved edge, which `query issues` counts and `check` reds at whatever severity `[[detection.unresolved_policy]]` gives the cause — there the warning says the rename gave up now rather than a build later.
