...
A `file_skipped` warning from `rename` splits by what the reference names afterwards. A **different valid document** is reported nowhere else: the write succeeded and the graph is valid, so the warning is the only record. **Nothing** surfaces on the next build as an unresolved edge that `query issues` counts — never as a `check` error, because a project whose `[[detection.unresolved_policy]]` calls that cause an error is one whose write gate refuses the move outright, and then there is no warning to read. So the warning says the rename gave up on a reference the project tolerates losing, a build before anything else mentions it.
