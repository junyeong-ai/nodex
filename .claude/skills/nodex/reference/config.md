...
Both rules judge history **a step at a time**: each commit `<ref>..HEAD` adds against its parents, and the uncommitted change against `HEAD` — and every `MERGE_HEAD` while a merge is under way. A range reports exactly what a gate on each of its commits would, so authoring at `proposed` and accepting a commit later passes, and a detour through an undeclared status is reported at the commit that took it. A merge is judged only where it differs from every parent. `details.commit` names the step; it is absent for the uncommitted change. A write seam judges the step its write would commit, from `HEAD`; `check --content` has no commit to step from and reports both rules skipped.

In each step a governed record the flow also governed on a parent is `status_transition`'s — its status must equal a parent's or be a declared move from one — and one it governed on no parent is `status_entry`'s. That second set is what **entering the flow** means, and a node is its id, so every way in reads the same: written here, moved under an id that follows its path, re-keyed, given a governed kind, or back after a commit that deleted it or could not parse it. The finding names the remedies — author at the entry status and move in a later commit, or give a document that already existed its record back by anchoring `id` in frontmatter or moving it with `nodex rename`, which anchors it. A record leaving the governed kinds is judged by neither rule.

The history judged is the one the repository keeps: a squash merge enters a record authored and accepted on the branch at its accepted status, and a range a shallow clone cuts is refused rather than read as every record arriving. Each commit in the range costs a build, so an `immutable_baseline` far behind `HEAD` makes every default `check` walk that far.
