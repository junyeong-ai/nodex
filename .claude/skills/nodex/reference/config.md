...
Both rules judge history **a step at a time**, against git rather than `rules.immutable_baseline`. Every `check`, `query issues` and write seam judges the uncommitted change against `HEAD` — and every `MERGE_HEAD` while a merge is under way; `check --since <ref>` also judges each commit the heads reach and `<ref>` does not, against its parents. A range reports what a gate on each of its commits would under today's config, so authoring at `proposed` and accepting a commit later passes, and a detour through an undeclared status is reported at the commit that took it. A merge is judged only where it differs from every parent, and stands on what each line moved since the parents last agreed — every place they agreed, where they agreed in more than one. A line that did not touch the record makes no claim about it, so a branch forked before a move cannot carry the old status back; lines sharing no commit, a record a step reads two ways, and a commit the build refuses — a parent or a place the lines agreed alike, and whether or not another parent carries the record — leave it counted rather than judged. `details.commit` names the step; it is absent for the uncommitted change. A step finding is about a commit, so a plain `check` does not re-judge history — a past finding belongs to the `--since` range that holds it. `lifecycle` and `scaffold --force` refuse a move the write would commit from `HEAD` as `INVALID_TRANSITION`, whatever the document carries uncommitted — including a write that changes no byte, since a status hand-written into the working tree and then confirmed through the seam commits the move `HEAD` records; `check --content` has no commit to step from and reports both rules skipped, as does any run outside a git work tree.
